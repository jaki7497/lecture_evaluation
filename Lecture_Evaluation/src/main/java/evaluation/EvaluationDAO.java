...
public class EvaluationDAO {
	...
	public int write (EvaluationDTO evaluationDTO) {
		...
		try {
			...
			pstmt.setString(3, evaluationDTO.getProfesseorName().replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\r\n",  "<br>"));
		...
		try {
			if (searchType.equals("최신순")) {
				SQL = "SELECT * FROM EVALUATION WHERE lectureDivide LIKE ? "
						+ "AND CONCAT (lectureName, professeorName, evaluationTitle, evaluationContent)"
...
//	강의 평가글 추천 삭제 기능 - 추천수 증가, 강의 평가글 삭제, 글 작성 유저 아이디 반환 함수 필요
